...
check-gpu:
	...
	export PYTHONNOUSERSITE=1; \
	export APPTAINER_USE_GPU=1; \
	TMP_DIAG=$$(mktemp /tmp/gpu_diag_$$$$.py); \
	echo "import sys, torch, os" > $$TMP_DIAG; \
	echo "print('=== Python Environment ===')" >> $$TMP_DIAG; \
	echo "print('Python version:', sys.version.split()[0])" >> $$TMP_DIAG; \
	echo "print('Python executable:', sys.executable)" >> $$TMP_DIAG; \
	echo "print('PATH:', os.environ.get('PATH', 'not set'))" >> $$TMP_DIAG; \
	echo "print('')" >> $$TMP_DIAG; \
	echo "print('=== PyTorch Information ===')" >> $$TMP_DIAG; \
	echo "print('PyTorch version:', torch.__version__)" >> $$TMP_DIAG; \
	echo "print('PyTorch CUDA available:', torch.cuda.is_available())" >> $$TMP_DIAG; \
	echo "if torch.cuda.is_available():" >> $$TMP_DIAG; \
	echo "    print('PyTorch CUDA version:', torch.version.cuda)" >> $$TMP_DIAG; \
	echo "else:" >> $$TMP_DIAG; \
	echo "    print('PyTorch CUDA version: N/A')" >> $$TMP_DIAG; \
	echo "print('')" >> $$TMP_DIAG; \
	echo "print('=== CUDA Device Detection ===')" >> $$TMP_DIAG; \
	echo "print('torch.cuda.device_count():', torch.cuda.device_count())" >> $$TMP_DIAG; \
	echo "print('torch.cuda.is_available():', torch.cuda.is_available())" >> $$TMP_DIAG; \
	echo "print('CUDA_VISIBLE_DEVICES:', os.environ.get('CUDA_VISIBLE_DEVICES', 'not set'))" >> $$TMP_DIAG; \
	echo "print('')" >> $$TMP_DIAG; \
	echo "if torch.cuda.is_available() and torch.cuda.device_count() > 0:" >> $$TMP_DIAG; \
	echo "    print('✓ CUDA devices detected:')" >> $$TMP_DIAG; \
	echo "    for i in range(torch.cuda.device_count()):" >> $$TMP_DIAG; \
	echo "        props = torch.cuda.get_device_properties(i)" >> $$TMP_DIAG; \
	echo "        print(f'  Device {i}: {props.name}')" >> $$TMP_DIAG; \
	echo "        print(f'    Total Memory: {props.total_memory / 1e9:.1f} GB')" >> $$TMP_DIAG; \
	echo "        print(f'    Compute Capability: {props.major}.{props.minor}')" >> $$TMP_DIAG; \
	echo "    print('')" >> $$TMP_DIAG; \
	echo "    print('✓ GPU is accessible inside container')" >> $$TMP_DIAG; \
	echo "    print('✓ PyTorch can detect CUDA devices')" >> $$TMP_DIAG; \
	echo "    print('✓ Container setup appears correct')" >> $$TMP_DIAG; \
	echo "else:" >> $$TMP_DIAG; \
	echo "    print('✗ GPU is NOT accessible inside container')" >> $$TMP_DIAG; \
	echo "    print('')" >> $$TMP_DIAG; \
	echo "    print('Diagnostics:')" >> $$TMP_DIAG; \
	echo "    print('  torch.cuda.is_available() =', torch.cuda.is_available())" >> $$TMP_DIAG; \
	echo "    print('  torch.cuda.device_count() =', torch.cuda.device_count())" >> $$TMP_DIAG; \
	echo "    print('')" >> $$TMP_DIAG; \
	echo "    print('Possible issues:')" >> $$TMP_DIAG; \
	echo "    print('  1. Container CUDA 11.7 may not be compatible with host driver')" >> $$TMP_DIAG; \
	echo "    print('     - Host shows CUDA 13.0 (forward compatible, should work)')" >> $$TMP_DIAG; \
	echo "    print('     - But MIG device might need special handling')" >> $$TMP_DIAG; \
	echo "    print('  2. MIG device UUID in CUDA_VISIBLE_DEVICES may not work with --nv')" >> $$TMP_DIAG; \
	echo "    print('  3. Try: export CUDA_VISIBLE_DEVICES=0 (if non-MIG GPU available)')" >> $$TMP_DIAG; \
	apptainer exec --nv \
		--bind "$(shell pwd)/data:/transcriptomic-fms/data" \
		--bind "$(shell pwd)/output:/transcriptomic-fms/output" \
		--bind "$(shell pwd)/models:/transcriptomic-fms/models" \
		--bind "$$TMP_DIAG:/tmp/gpu_diag.py:ro" \
		"$$CONTAINER" \
		python /tmp/gpu_diag.py || \
	apptainer exec --nv \
		--bind "$(shell pwd)/data:/transcriptomic-fms/data" \
		--bind "$(shell pwd)/output:/transcriptomic-fms/output" \
		--bind "$(shell pwd)/models:/transcriptomic-fms/models" \
		--bind "$$TMP_DIAG:/tmp/gpu_diag.py:ro" \
		"$$CONTAINER" \
		/usr/bin/python3 /tmp/gpu_diag.py || \
	apptainer exec --nv \
		--bind "$(shell pwd)/data:/transcriptomic-fms/data" \
		--bind "$(shell pwd)/output:/transcriptomic-fms/output" \
		--bind "$(shell pwd)/models:/transcriptomic-fms/models" \
		...
		"$$CONTAINER" \
		sh -c 'python /tmp/gpu_diag.py || python3 /tmp/gpu_diag.py || /usr/bin/python3 /tmp/gpu_diag.py'; \
	rm -f $$TMP_DIAG; \
	echo ""; \
